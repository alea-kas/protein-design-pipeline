## 1. Python‑скрипт `src/generate_chroma_motif_scaffolds.py`

Этот файл одинаковый и для Windows, и для Linux:
...
DATA_DIR = Path("data")          # общий для Windows и Linux
...
def generate_with_motif(
    ...
    while len(records) < n_wanted:
        ...
        # Сохраняем во временный FASTA и читаем обратно, чтобы знать API
...
if __name__ == "__main__":
    ...
    generate_with_motif(
        motif=motif,
        chain_length=60,      # можно менять длину аминокислотной последовательности
        n_wanted=10,
        batch_size=20,
...
- не зависит от тонкостей `protein.to_sequence()` — использует `protein.to_fasta`, как в README.
- даёт реальные последовательности с мотивом, которые дальше можно прогонять через остальные шаги конвейера.

***

## 2. Запуск на Windows

Батник в корне репозитория, например `run_chroma_win.bat`:
...
Скрипт `run_chroma_linux.sh` в корне репозитория:

```bash
#!/bin/bash
set -e

# если conda не инициализирована автоматически, раскомментируй:
...
```bash
chmod +x run_chroma_linux.sh
bash run_chroma_linux.sh
```
